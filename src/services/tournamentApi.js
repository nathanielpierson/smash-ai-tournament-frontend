...
const PRODUCTION_API_BASE =
  "https://smash-ai-tournament-backend.onrender.com/api";
const PRODUCTION_ORIGIN = "https://smash-ai-tournament-backend.onrender.com";
...
export const fetchTournamentData = async () => {
  ...
  try {
    ...
    if (matchupsResponse.status === "fulfilled" && matchupsResponse.value.ok) {
      ...
      const reason =
        matchupsResponse.status === "rejected"
          ...
          : matchupsResponse.value?.statusText ||
            `HTTP ${matchupsResponse.value?.status}`;
    ...
    if (
      contestantsResponse.status === "fulfilled" &&
      contestantsResponse.value &&
    ...
    if (Array.isArray(contestants) && contestants.length > 0) {
      ...
      if (formattedData.matchups.length > 0) {
        ...
        console.log(
          "Matchup contestant_one_id:",
          sampleMatchup.contestant_one_id,
        );
        console.log(
          "Matchup contestant_two_id:",
          sampleMatchup.contestant_two_id,
        );
      }
    ...
    if (Array.isArray(data)) {
      ...
      normalizedMatchups.forEach((matchup) => {
        if (matchup.contestant_one_id)
          contestantIds.add(matchup.contestant_one_id);
        if (matchup.contestant_two_id)
          contestantIds.add(matchup.contestant_two_id);
      });
      ...
      data.forEach((matchup) => {
        if (
          matchup.contestant_one &&
          !extractedContestants.find((c) => c.id === matchup.contestant_one.id)
        ) {
          extractedContestants.push(matchup.contestant_one);
        }
        if (
          matchup.contestant_two &&
          !extractedContestants.find((c) => c.id === matchup.contestant_two.id)
        ) {
      ...
      const formattedData = {
        matchups: normalizedMatchups,
        contestants:
          extractedContestants.length > 0 ? extractedContestants : [], // Will need to fetch separately if not in matchups
      };
    ...
    if (formattedData.matchups.length > 0) {
      console.log(
        "Sample matchup (after normalization):",
        formattedData.matchups[0],
      );
    }
    return formattedData;
  } catch (error) {
    ...
    console.error(
      "If you see this, the app is not receiving data from the backend. Check:",
    );
    console.error("  1. Backend is running (e.g. rails s)");
    console.error("  2. API base URL is correct. Current:", API_BASE_URL);
    console.error(
      "  3. CORS allows your frontend origin (if frontend and backend are different ports)",
    );
    console.error(
      "  4. Routes exist: GET /api/matchups and GET /api/contestants",
    );
